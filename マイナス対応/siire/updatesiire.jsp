...
<script language="JavaScript">
	...
	function deleteUpdateSiire(pForm){
		if (confirm("削除してよろしいでしょうか？")==true){
			document.SiireForm.action="deleteUpdateSiire.action";
			document.SiireForm.submit();
		}
	}
	function deleteUpdateSiireTop(pForm){
		if (confirm("削除してよろしいでしょうか？")==true){
			document.SiireForm.action="deleteUpdateSiireTop.action";
			document.SiireForm.submit();
		}
	}	
	...
	function deleteUpdateSiireFromProject(pForm){
		if (confirm("削除してよろしいでしょうか？")==true){
			document.SiireForm.action="deleteUpdateSiireFromProject.action";
			document.SiireForm.submit();
		}
	}
              ...
              <div class="form-group">
                <div>			
					<ww:if test="form.msgId=='top'">
						...
						<input type="button" class="btn btn-primary dora-sm-button" value="保存" onclick="submitAction(this.form,'saveUpdateSiireTop.action');"/>
...
<!-- 						<input type="button" class="btn btn-primary dora-sm-button" value="保存" onclick="submitAction(this.form,'saveUpdateSiire.action');"/> -->
<!-- 						<input type="button" class="btn btn-primary dora-sm-button" value="削除" onclick="deleteUpdateSiire(this.form);"/> -->
						<!-- sxt 20220721 del end -->
						
						<!-- sxt 20220721 add start -->
						<ww:if test="form.modoruFlg.equals(\"1\")">
						    <input type="button" class="btn btn-primary dora-sm-button" value="保存" onclick="submitAction(this.form,'saveUpdateSiireFromProject.action');"/>
                  ...
                  <div id="myTabContent" class="tab-content">
                    <div role="tabpanel" class="tab-pane fade active in panel-body" id="home" aria-labelledby="home-tab">
                      <div class="form-group form-inline">
                        ...
                        <label class="dora-label-normal btn-link" onclick="linkAction('showHattyuItiran.action?form.order_no_itiran=<ww:property  value="form.order_no_update" />&form.page_flg=7','');"><ww:property  value="form.order_no_update" /></label>
...
// 	 		            $("#tbAddPart").append($("#tbAddPart tr").eq(1).clone());
			...
			$("#deleteRow").click(function(){
				...
				if (deleteMeisaiUpdateSiire()) {
					$.ajax({
				        type: "POST",  
				        url: "deleteMeisaiUpdateSiire.action",
