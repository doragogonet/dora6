...
<!-- 					<button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'saveSinkiSiire2.action');">保存</button> -->
					<!-- sxt 20220712 del end -->
					<!-- sxt 20220721 add start -->
					<ww:if test="form.modoruFlg.equals(\"1\")">
					    <button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'saveSinkiSiire2FromProject.action');">保存</button>
					</ww:if>
					<ww:else> 
					    <button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'saveSinkiSiire2.action');">保存</button>
					</ww:else>
                  ...
                  <div id="myTabContent" class="tab-content">
                    <div role="tabpanel" class="tab-pane fade active in panel-body" id="home" aria-labelledby="home-tab">
                      <div class="form-group form-inline">
                        ...
                        <label class="dora-label-normal btn-link" onclick="linkAction('showHattyuItiran2.action?form.order_no_itiran=<ww:property  value="form.order_no" />&form.page_flg=6','');"><ww:property  value="form.order_no" /></label>
...
<%-- 	                            	<ww:hidden name="'form.stockDetailModelList['+#rows.index+'].more_price'" value="form.stockDetailModelList[#rows.index].more_price" /> --%>
<%-- 	                            	<ww:hidden name="'form.stockDetailModelList['+#rows.index+'].less_price'" value="form.stockDetailModelList[#rows.index].less_price" /> --%>
									...
									<input type="button" value="参照" class="btn btn-primary dora-tabel-button" 
										onclick="openWindowWithScrollbarGuide('initialSagyousyaGuide.action?form.fieldName_code=form.stockDetailModelList[<%=s%>].task_code&form.fieldName_name=form.stockDetailModelList[<%=s%>].task_content&form.company_name=form.stockDetailModelList[<%=s%>].company_name&form.sagyousya_flg=2','技術者参照',670,716)">		
	                            </div>
                            </td>
							...
							<ww:if test="time_kbn.equals('01')">
								<td class="text-center" rowspan="2">
									<input type="number" name="form.stockDetailModelList[<ww:property value='#rows.index'/>].calculate_amount" 
										value="<ww:property value='form.stockDetailModelList[#rows.index].calculate_amount'/>" class="form-control text-right"
										oninput="maxNumberLength(8)" onKeypress="return integerOnly()"/>
								</td>
							</ww:if>
							<ww:else>
...
<!--                             </td> -->
                            ...
                            <td rowspan="2">
                            	<!-- sxt 20221024 add start -->
								<ww:if test="time_kbn.equals('')">
									<input type="number" name="form.stockDetailModelList[<ww:property value='#rows.index'/>].carfare" 
											value="<ww:property value='form.stockDetailModelList[#rows.index].carfare'/>" class="form-control text-right"
											readonly style="background-color:#fff"/>
								</ww:if>
								<ww:else>
								<!-- sxt 20221024 add end -->
                            ...
                            <td class="text-center" rowspan="2">
								<ww:textarea name="'form.stockDetailModelList['+#rows.index+'].biko'" value="biko" cssClass="'form-control'" cssStyle="'height:4.5em'"></ww:textarea>
							</td>
                          </tr>
                          <tr>
...
<script language="JavaScript">
		...
		function setItemState(isReadOnly,index){
			if (isReadOnly){
				...
				$("input[name='form.stockDetailModelList[" + index +"].calculate_amount'").attr("readonly","true");
				$("input[name='form.stockDetailModelList[" + index +"].calculate_amount'").attr("style","background-color:#fff");
				$("input[name='form.stockDetailModelList[" + index +"].calculate_amount'").val("");
				
				$("input[name='form.stockDetailModelList[" + index +"].carfare'").attr("readonly","true");
				$("input[name='form.stockDetailModelList[" + index +"].carfare'").attr("style","background-color:#fff");
				$("input[name='form.stockDetailModelList[" + index +"].carfare'").val("");
				...
				$("input[name='form.stockDetailModelList[" + index +"].calculate_amount'").removeAttr("readonly");
				$("input[name='form.stockDetailModelList[" + index +"].calculate_amount'").removeAttr("style");
				
				$("input[name='form.stockDetailModelList[" + index +"].carfare'").removeAttr("readonly");
				$("input[name='form.stockDetailModelList[" + index +"].carfare'").removeAttr("style");
			}
		}
		//sxt 20221025 add end
		...
		function setCalculateAmount(index){
			...
			if ($("select[name='form.stockDetailModelList[" + index +"].time_kbn'").val() == "01"){		
				...
				if (quantity > work_time_to) {
					...
					var morePrice = parseFloat($("input[name='form.stockDetailModelList[" + index +"].more_price'").val());
				...
				if (quantity < work_time_from) {
					...
					var lessPrice = parseFloat($("input[name='form.stockDetailModelList[" + index +"].less_price'").val());
				...
				if (calculateAmount != 0) {
					$("input[name='form.stockDetailModelList[" + index +"].calculate_amount'").val(calculateAmount);	
				} else {
					$("input[name='form.stockDetailModelList[" + index +"].calculate_amount'").val("");	
				}
		...
		function setAmount(index){
			...
			//精算額
			var calculateAmount = 0;
			if ($("input[name='form.stockDetailModelList[" + index +"].calculate_amount'").val()){
				calculateAmount = parseFloat($("input[name='form.stockDetailModelList[" + index +"].calculate_amount'").val());
			}
			...
			//そのた
			var otherPrice = 0;
			if ($("input[name='form.stockDetailModelList[" + index +"].other_price'").val()){
				otherPrice = parseFloat($("input[name='form.stockDetailModelList[" + index +"].other_price'").val());
			}
		...
		function calculateTotal(){
			...
			for (var i = 0; i < detailsize; i++) {
				...
				//明細交通費
				var carfareDetail = 0; ;	
				if ($("input[name='form.stockDetailModelList[" + i +"].carfare'").val()) {
					carfareDetail = parseInt($("input[name='form.stockDetailModelList[" + i +"].carfare'").val());
				} 
				
				//明細税区分
				var tax_div = $("select[name='form.stockDetailModelList[" + i +"].tax_div'").val();
