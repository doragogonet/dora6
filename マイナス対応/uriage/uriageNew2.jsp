...
<!-- 					<button type="button" class="btn btn-primary dora-sm-button" onclick="formSubmitDblClkChk(this.form,'saveUriage2.action');">保存</button>	 -->
					<!-- sxt 20220721 del end -->
					<!-- sxt 20220721 add start -->
					<ww:if test="form.prepage.equals(\"7\")">
						<button type="button" class="btn btn-primary dora-sm-button" onclick="formSubmitDblClkChk(this.form,'saveUriage2FromProject.action');">保存</button>	
					</ww:if>	
					<ww:else>
						<button type="button" class="btn btn-primary dora-sm-button" onclick="formSubmitDblClkChk(this.form,'saveUriage2.action');">保存</button>
					</ww:else>
                    ...
                    <div id="myTabContent" class="tab-content">
                      <div role="tabpanel" class="tab-pane fade active in panel-body" id="home" aria-labelledby="home-tab">
                        <div class="form-group form-inline">
                          <span>
                            見積番号
                            <button type="button" class="btn btn-link" onclick="hrefHaveParamAction('mitumoriInfo2.action?form.estimate_no=<ww:property value="form.estimate_no" />&form.pageFlg=5')">
                          		<ww:property value="form.estimate_no" />
                          	</button>
                          </span>
                          <span>
                            受注番号
                            <button type="button" class="btn btn-link" onclick="hrefHaveParamAction('jyutyuInfo2.action?form.pageFlg=2&form.receive_no=<ww:property value="form.received_no_input" />');">
...
<!-- 								<td class="text-center" style="width:10rem;" rowspan="2">そのた（円）</td> -->	<!-- sxt 20221024 del -->
	                        ...
	                        <tbody id="tbAddPart">
	                        <%int s = 0; %>
	                        <ww:iterator value="form.receivedDetailList" status="rows">
	                          <tr>
	                            ...
	                            <td class="text-center" style="border-bottom: none;">
		                            <div class="flex">
		                            	<ww:hidden name="'form.receivedDetailList['+#rows.index+'].task_code'" value="form.receivedDetailList[#rows.index].task_code" />
	                              		<input type="text" name="form.receivedDetailList[<ww:property value='#rows.index'/>].task_content" 
											value='<ww:property value="form.receivedDetailList[#rows.index].task_content"/>' class="form-control" />
										<input type="button" value="参照" class="btn btn-primary dora-tabel-button" 
											onclick="openWindowWithScrollbarGuide('initialSagyousyaGuide.action?form.fieldName_code=form.receivedDetailList[<%=s%>].task_code&form.fieldName_name=form.receivedDetailList[<%=s%>].task_content&form.company_name=form.receivedDetailList[<%=s%>].company_name&form.sagyousya_flg=2','技術者参照',670,716)">		
		                            </div>
	                            </td>
	                            <td rowspan="2" class="text-center" style="width:6.75rem;" nowrap>
									<ww:select name="'form.receivedDetailList['+#rows.index+'].time_kbn'"
										       cssClass="'form-control'" 
								...
								<ww:if test="time_kbn.equals('01')">
									<td rowspan="2">
		                              <input type="number" name="form.receivedDetailList[<ww:property value='#rows.index'/>].more_price" 
											value="<ww:property value='form.receivedDetailList[#rows.index].more_price'/>" class="form-control text-right"
											oninput="maxNumberLength(8)" onKeypress="return integerOnly()"/>
		                            </td>
	                            </ww:if>
								<ww:else>
	                            ...
	                            <ww:if test="time_kbn.equals('01')">
	                            	<td rowspan="2">
		                              <input type="number" name="form.receivedDetailList[<ww:property value='#rows.index'/>].less_price" 
											value="<ww:property value='form.receivedDetailList[#rows.index].less_price'/>" class="form-control text-right"
											oninput="maxNumberLength(8)" onKeypress="return integerOnly()"/>
		                            </td>
								</ww:if>
								<ww:else>
								...
								<ww:if test="time_kbn.equals('01')">
									<td class="text-center" rowspan="2">
										<input type="number" name="form.receivedDetailList[<ww:property value='#rows.index'/>].calculate_amount" 
											value="<ww:property value='form.receivedDetailList[#rows.index].calculate_amount'/>" class="form-control text-right"
											oninput="maxNumberLength(8)" onKeypress="return integerOnly()"/>
									</td>
								</ww:if>
								<ww:else>
...
<!-- 	                            <td rowspan="2"> -->
<%-- 	                              <input type="number" name="form.receivedDetailList[<ww:property value='#rows.index'/>].other_price"  --%>
<%-- 										value="<ww:property value='form.receivedDetailList[#rows.index].other_price'/>" class="form-control text-right" --%>
<!-- 										oninput="maxNumberLength(8)" onKeypress="return integerOnly()"/> -->
<!-- 	                            </td> -->
	                            ...
	                            <td rowspan="2">
	                            	<!-- sxt 20221024 add start -->
	                            	<input type="hidden" name="form.receivedDetailList[<ww:property value='#rows.index'/>].other_price" 
										value="<ww:property value='form.receivedDetailList[#rows.index].other_price'/>" />
	                            	<!-- sxt 20221024 add end -->
	                            ...
	                            <td rowspan="2">
	                            	<!-- sxt 20221024 add start -->
									<ww:if test="time_kbn.equals('')">
										<input type="number" name="form.receivedDetailList[<ww:property value='#rows.index'/>].carfare" 
											value="<ww:property value='form.receivedDetailList[#rows.index].carfare'/>" class="form-control text-right"
											readonly style="background-color:#fff"/>
									</ww:if>
									<ww:else>
									<!-- sxt 20221024 add end -->
	                            ...
	                            <td class="text-center" rowspan="2">
									<ww:textarea name="'form.receivedDetailList['+#rows.index+'].biko'" value="biko" cssClass="'form-control'" cssStyle="'height:4.5em'"></ww:textarea>
								</td>
	                          </tr>
	                          <tr>
	                            <td style="border-top: none;">
									<input type=text disabled name="form.receivedDetailList[<ww:property value='#rows.index'/>].company_name" 
										value="<ww:property value='form.receivedDetailList[#rows.index].company_name'/>" style="background-color: #ffffff;" />	 
								</td>
	                          </tr>
	                          <%  s++; %>
	                          </ww:iterator>
	...
	<script language="JavaScript">
		$(document).ready(function(){
			...
			$("input[name$='task_content']").blur(function(){
				...
				if (!$(this).val()){
					...
					var index = getCurIndex(x);
	 				...
	 				//技術者コードをクリアする
	 				$("input[name='form.receivedDetailList[" + index +"].task_code'").val("");
	 				
	 				//会社名をクリアする
	 				$("input[name='form.receivedDetailList[" + index +"].company_name'").val("");
				}
			});
			...
			$("select[name$='time_kbn']").change(function(){
				...
				//空白を選択する時
				if ($("select[name='form.receivedDetailList[" + index +"].time_kbn'").val() == ""){	
		...
		function setItemState(isReadOnly,index){
			if (isReadOnly){
				$("input[name='form.receivedDetailList[" + index +"].price_per'").attr("readonly","true");
				$("input[name='form.receivedDetailList[" + index +"].price_per'").attr("style","background-color:#fff");
				$("input[name='form.receivedDetailList[" + index +"].price_per'").val("");
				
				$("input[name='form.receivedDetailList[" + index +"].quantity'").attr("readonly","true");
				$("input[name='form.receivedDetailList[" + index +"].quantity'").attr("style","background-color:#fff");
				$("input[name='form.receivedDetailList[" + index +"].quantity'").val("");
				
				$("input[name='form.receivedDetailList[" + index +"].more_price'").attr("readonly","true");
				$("input[name='form.receivedDetailList[" + index +"].more_price'").attr("style","background-color:#fff");
				$("input[name='form.receivedDetailList[" + index +"].more_price'").val("");
				
				$("input[name='form.receivedDetailList[" + index +"].less_price'").attr("readonly","true");
				$("input[name='form.receivedDetailList[" + index +"].less_price'").attr("style","background-color:#fff");
				$("input[name='form.receivedDetailList[" + index +"].less_price'").val("");
				
				$("input[name='form.receivedDetailList[" + index +"].calculate_amount'").attr("readonly","true");
				$("input[name='form.receivedDetailList[" + index +"].calculate_amount'").attr("style","background-color:#fff");
				$("input[name='form.receivedDetailList[" + index +"].calculate_amount'").val("");
				
				$("input[name='form.receivedDetailList[" + index +"].carfare'").attr("readonly","true");
				$("input[name='form.receivedDetailList[" + index +"].carfare'").attr("style","background-color:#fff");
				$("input[name='form.receivedDetailList[" + index +"].carfare'").val("");
				
			} else {
				$("input[name='form.receivedDetailList[" + index +"].price_per'").removeAttr("readonly");
				$("input[name='form.receivedDetailList[" + index +"].price_per'").removeAttr("style");
				
				$("input[name='form.receivedDetailList[" + index +"].quantity'").removeAttr("readonly");
				$("input[name='form.receivedDetailList[" + index +"].quantity'").removeAttr("style");
				
				$("input[name='form.receivedDetailList[" + index +"].more_price'").removeAttr("readonly");
				$("input[name='form.receivedDetailList[" + index +"].more_price'").removeAttr("style");
				
				$("input[name='form.receivedDetailList[" + index +"].less_price'").removeAttr("readonly");
				$("input[name='form.receivedDetailList[" + index +"].less_price'").removeAttr("style");
				
				$("input[name='form.receivedDetailList[" + index +"].calculate_amount'").removeAttr("readonly");
				$("input[name='form.receivedDetailList[" + index +"].calculate_amount'").removeAttr("style");
				
				$("input[name='form.receivedDetailList[" + index +"].carfare'").removeAttr("readonly");
				$("input[name='form.receivedDetailList[" + index +"].carfare'").removeAttr("style");
			}
		}
		//sxt 20221025 add end
		...
		function setCalculateAmount(index){
			//月を選択する時
			if ($("select[name='form.receivedDetailList[" + index +"].time_kbn'").val() == "01"){		
				...
				if (quantity > work_time_to) {
					...
					var morePrice = parseFloat($("input[name='form.receivedDetailList[" + index +"].more_price'").val());
				...
				if (quantity < work_time_from) {
					...
					var lessPrice = parseFloat($("input[name='form.receivedDetailList[" + index +"].less_price'").val());
				...
				if (calculateAmount != 0) {
					$("input[name='form.receivedDetailList[" + index +"].calculate_amount'").val(calculateAmount);	
				} else {
					$("input[name='form.receivedDetailList[" + index +"].calculate_amount'").val("");	
				}

			} else if ($("select[name='form.receivedDetailList[" + index +"].time_kbn'").val() == "02"){		
				$("input[name='form.receivedDetailList[" + index +"].calculate_amount'").val("");
			}
		}
		...
		function setAmount(index){
			...
			//精算額
			var calculateAmount = 0;
			if ($("input[name='form.receivedDetailList[" + index +"].calculate_amount'").val()){
				calculateAmount = parseFloat($("input[name='form.receivedDetailList[" + index +"].calculate_amount'").val());
			}
			...
			//そのた
			var otherPrice = 0;
			if ($("input[name='form.receivedDetailList[" + index +"].other_price'").val()){
				otherPrice = parseFloat($("input[name='form.receivedDetailList[" + index +"].other_price'").val());
			}
		...
		function calculateTotal(){
			...
			for (var i = 0; i < detailsize; i++) {
				...
				//明細交通費
				var carfareDetail = 0; ;	
				if ($("input[name='form.receivedDetailList[" + i +"].carfare'").val()) {
					carfareDetail = parseInt($("input[name='form.receivedDetailList[" + i +"].carfare'").val());
				} 
