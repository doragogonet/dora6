...
<!-- 					<input type="button" class="btn btn-primary dora-sm-button" value="戻る" onclick="submitFunction(this.form,'refreshNewUriage.action')">  -->
					...
					<input type="button" class="btn btn-primary dora-sm-button" value="保存" onclick="formSubmitDblClkChk(this.form,'saveTopEditUriage.action')">	
					</ww:if>
					<!-- sxt 20220721 add start -->
					<ww:elseif test="form.prepage.equals(\"7\")">
						<input type="button" class="btn btn-primary dora-sm-button" value="保存" onclick="formSubmitDblClkChk(this.form,'saveEditUriageFromProject.action')">
					</ww:elseif>
					<!-- sxt 20220721 add end -->
					<ww:else>
					<input type="button" class="btn btn-primary dora-sm-button" value="保存" onclick="formSubmitDblClkChk(this.form,'saveEditUriage.action')">	
					</ww:else>	
					<ww:if test="form.prepage.equals(\"0\")">
                    ...
                    <div id="myTabContent" class="tab-content">
                      <div role="tabpanel" class="tab-pane fade active in panel-body" id="home" aria-labelledby="home-tab">
                        <div class="form-group form-inline">
                          <span>
                            見積番号
                            <button type="button" class="btn btn-link" onclick="hrefHaveParamAction('mitumoriInfo.action?form.estimate_no=<ww:property value="form.estimate_no" />&form.pageFlg=6')">
                          		<ww:property value="form.estimate_no" />
                          	</button>
                          </span>
                          <span>
                            受注番号
                            <button type="button" class="btn btn-link" onclick="hrefHaveParamAction('jyutyuInfo.action?form.pageFlg=3&form.receive_no=<ww:property value="form.received_no_input" />');">
                          ...
                          <tbody id="tbAddPart">
                            ...
                            <tr>
                              ...
                              <td class="text-center" rowspan="2">
                                <ww:hidden name="'form.salesDetailList['+#rows.index+'].task_code'" />
				  			    <ww:textarea name="'form.salesDetailList['+#rows.index+'].task_content'" value="task_content" cssClass="'control'" rows="3"></ww:textarea>
                              </td>
                              <td class="text-center" rowspan="2" style="width:6rem;">
                                <input type="button" value="参照" class="btn btn-primary dora-tabel-button" onclick="openWindowWithScrollbarGuide('initSagyounaiyouGuide.action?form.fieldName_code=form.salesDetailList[<%=s%>].task_code&form.fieldName_name=form.salesDetailList[<%=s%>].task_content','作業内容参照',670,600)">
                              </td>
                              ...
                              <td class="text-center" >
                                <ww:select name="'form.salesDetailList['+#rows.index+'].unit_div'"
									       cssClass="'form-control'" 
                              ...
                              <td class="text-center"  rowspan="2">
                                <ww:checkbox name="'form.salesDetailList['+#rows.index+'].checkbox'" fieldValue="checkbox" ></ww:checkbox>
                              </td>
                            </tr>
                            <tr>
                              ...
                              <td class="text-center" ></td>
                              <td class="text-center" >
                                <ww:select name="'form.salesDetailList['+#rows.index+'].tax_div'"
								       	   cssClass="'form-control'" 
                              ...
                              <td class="text-center" >
                                <input type="number" name="form.salesDetailList[<ww:property value='#rows.index'/>].org_price" 
									value="<ww:property value='form.salesDetailList[#rows.index].org_price'/>" class="form-control text-right"
									oninput="maxNumberLength(8)" onKeypress="return integerOnly()"/> 
									
								<ww:hidden name="'form.salesDetailList['+#rows.index+'].reg_id'" value="reg_id"></ww:hidden>
								<ww:hidden name="'form.salesDetailList['+#rows.index+'].reg_name'" value="reg_name"></ww:hidden>
								<ww:hidden name="'form.salesDetailList['+#rows.index+'].reg_date'" value="reg_date"></ww:hidden>
                              </td>
                            </tr>
		...
		$(document).ready(function(){
			...
			$("#addRow").click(function(){
				...
				function addRow(){
					...
					//最大行番号を取得する
					var maxRowNumber = parseInt($("input[name='form.salesDetailList[" + (size-1) +"].row_number'").val());
			...
			function appendRow(n,rowNumber){
				...
				//设置input标签name
				row_number.attr("name","form.salesDetailList[" + n + "].row_number");
				//设置input标签value
				...
				task_code.attr("name","form.salesDetailList[" + n + "].task_code");
				task_code.attr("value",""); 
				task_code.val("");
				
				var task_content = col1.find("textarea");
				task_content.attr("name","form.salesDetailList[" + n + "].task_content");
				...
				//获得当前TD的input标签 【参照】
				var clickContent = "openWindowWithScrollbarGuide('initSagyounaiyouGuide.action?form.fieldName_code=form.salesDetailList[" + n + "].task_code&form.fieldName_name=form.salesDetailList[" + n+ "].task_content','作業内容参照',670,600)";
				...
				price_per.attr("name","form.salesDetailList[" + n + "].price_per");
				...
				quantity.attr("name","form.salesDetailList[" + n + "].quantity");
				quantity.attr("value",""); 
				quantity.val("");
				...
				unit_div.attr("name","form.salesDetailList[" + n + "].unit_div");
				unit_div.attr("value",""); 
				unit_div.val("");
				...
				amount.attr("name","form.salesDetailList[" + n + "].amount");
				amount.attr("value",""); 
				amount.val("");
				...
				checkbox.attr("name","form.salesDetailList[" + n + "].checkbox");
				...
				tax_div.attr("name","form.salesDetailList[" + n + "].tax_div");
				tax_div.attr("value",""); 
				tax_div.val("");
				...
				org_price.attr("name","form.salesDetailList[" + n + "].org_price");
					...
					$.ajax({
				        ...
				        success: function (data) {  	
				        	...
				        	$("input[name$='checkbox']:checked").each(function() { 	
								...
								$("input[name^='form.salesDetailList'][name$='row_number']").each(function() {
									//获取当前元素的name
			...
			function reSetRowIndex(curIndex){
				...
				$("input[name^='form.salesDetailList'][name$='row_number']").each(function() {
					...
					if (index > curIndex){
						...
						$("input[name='form.salesDetailList[" + index +"].row_number'").attr("name","form.salesDetailList[" + (index-1) + "].row_number");
						$("input[name='form.salesDetailList[" + index +"].task_code'").attr("name","form.salesDetailList[" + (index-1) + "].task_code");
						$("textarea[name='form.salesDetailList[" + index +"].task_content'").attr("name","form.salesDetailList[" + (index-1) + "].task_content");
						
						var clickContent = "openWindowWithScrollbarGuide('initSagyounaiyouGuide.action?form.fieldName_code=form.salesDetailList[" + (index-1) + "].task_code&form.fieldName_name=form.salesDetailList[" + (index-1) + "].task_content','作業内容参照',670,600)";		
						$("input[name='button[" + index +"]'").attr("onclick",clickContent);
						$("input[name='button[" + index +"]'").attr("name","button[" + (index-1) + "]");
							
						$("input[name='form.salesDetailList[" + index +"].price_per'").attr("name","form.salesDetailList[" + (index-1) + "].price_per");
						$("input[name='form.salesDetailList[" + index +"].quantity'").attr("name","form.salesDetailList[" + (index-1) + "].quantity");
						$("select[name='form.salesDetailList[" + index +"].unit_div'").attr("name","form.salesDetailList[" + (index-1) + "].unit_div");
						$("input[name='form.salesDetailList[" + index +"].amount'").attr("name","form.salesDetailList[" + (index-1) + "].amount");
						$("input[name='form.salesDetailList[" + index +"].checkbox'").attr("name","form.salesDetailList[" + (index-1) + "].checkbox");
						$("select[name='form.salesDetailList[" + index +"].tax_div'").attr("name","form.salesDetailList[" + (index-1) + "].tax_div");
						$("input[name='form.salesDetailList[" + index +"].org_price'").attr("name","form.salesDetailList[" + (index-1) + "].org_price");
					}
