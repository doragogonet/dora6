	...
	function optionDeleteRow(pForm){
		...
		if (count == 0){
			...
			if ( confirm("削除してよろしいでしょうか？") ){
				//sxt 20220614 del start
				//document.HattyuForm.action="hattyuOptionDeleteRow2.action";
				//document.HattyuForm.submit();
				//sxt 20220614 del end
				return true;	//sxt 20220614 add
			}
...
// 		submitAction(pForm,'calculateOrderDate2.action')
...
<!--                   <button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'saveSinkiHattyu2.action');">保存</button> -->
                  <!-- sxt 20220721 del end -->
                  <!-- sxt 20220721 add start -->
				  <ww:if test="form.modoruFlg.equals(\"1\")">
				    <button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'saveSinkiHattyu2FromProject.action');">保存</button>
				  </ww:if>
				  <ww:else> 
				    <button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'saveSinkiHattyu2.action');">保存</button>
				  </ww:else>
...
<%--                  <div class="dora-state-zone">請求区分　<ww:property value='form.order_div_name'/></div> --%>	<!-- sxt 20221008 del -->
				 ...
				 <ww:hidden name="'form.order_div_input_sinki'" value="'01'"></ww:hidden>
				 <!-- sxt 20221008 add end -->
                ...
                <div class="panel panel-default">
                  <div id="myTabContent" class="tab-content">
                    <div role="tabpanel" class="tab-pane fade active in panel-body" id="header" aria-labelledby="header-tab">
                      ...
                      <div class="form-group form-inline">
                        <label class="dora-label-left-require">外注先コード</label>
...
<!-- 							単月 -->
<%-- 							<ww:hidden name="'form.order_div_input_sinki'" value="'01'"></ww:hidden>	<!-- sxt 20220930 add --> --%>
<!--                       </div> -->
					  <!-- sxt 20221008 del end -->
...
<!-- 											style="width: 15rem;vertical-align:middle;">　</td> -->
									...
									<tr>
										...
										<td style="border:0;padding: 0.5rem 0.5rem 0 0.5rem;">
											<input type="text" id="work_time_to1" class="form-control text-right"
									...
									<tr>
										...
										<td style="border:0;padding: 0 0.5rem 0.75rem 0.5rem;">
											...
											<input type="text" id="work_time_to2" class="form-control text-right"
												style="height:1rem;width:6rem;display:inline;background-color:#fff" readonly>
											時間
										</td>
									</tr>
									...
									<tr>
										...
										<td style="border-bottom:0;padding: 0.5rem 0.5rem 0 0.5rem;">
											<input type="text" id="work_time_from1" class="form-control text-right"
												style="height:1rem;width:6rem;display:inline;background-color:#fff" readonly>
											時間に満たない時間数を控除時間とする。
										</td>
									</tr>
									<tr>
										...
										<td style="border:0;padding: 0 0.5rem 0.75rem 0.5rem;">
											控除単価：月額単価<span
												style="font:1rem Verdana,Arial,Tahoma;"> ÷ </span>
											<input type="text" id="work_time_from2" class="form-control text-right"
												style="height:1rem;width:6rem;display:inline;background-color:#fff" readonly>
											時間
										</td>
									</tr>
...
<!-- 							<td class="text-center" style="width:5rem;" rowspan="2">削除</td> -->
                        ...
                        <tbody id="tbAddPart">
                        <%int s = 0; %>
                        <ww:iterator value="form.stockDetailModelList" status="rows">
                          <tr>
                            ...
                            <td class="text-center" style="border-bottom: none;">
	                            <div class="flex">
	                            	<ww:hidden name="'form.stockDetailModelList['+#rows.index+'].task_code'" value="form.stockDetailModelList[#rows.index].task_code" />
<%-- 	                            	<ww:hidden name="'form.stockDetailModelList['+#rows.index+'].task_code_old'" /> --%>
                              		<input type="text" name="form.stockDetailModelList[<ww:property value='#rows.index'/>].task_content" 
										value='<ww:property value="form.stockDetailModelList[#rows.index].task_content"/>' class="form-control" />
									<input type="button" value="参照" class="btn btn-primary dora-tabel-button" name="form.stockDetailModelList[<ww:property value='#rows.index'/>].btn" />
								</div>
                            </td>
                            <td rowspan="2" class="text-center" style="width:6.75rem;" nowrap>
                              	<ww:select name="'form.stockDetailModelList['+#rows.index+'].time_kbn'"
								       cssClass="'form-control'" 
                            ...
                            <ww:if test="time_kbn.equals('01')">
                            	<td rowspan="2" class="text-center">
									<input type="number" name="form.stockDetailModelList[<ww:property value='#rows.index'/>].more_price" 
										value="<ww:property value='form.stockDetailModelList[#rows.index].more_price'/>" class="form-control text-right"
										oninput="maxNumberLength(8)" onKeypress="return integerOnly()"/>
								</td>
								<td rowspan="2" class="text-center">
									<input type="number" name="form.stockDetailModelList[<ww:property value='#rows.index'/>].less_price" 
										value="<ww:property value='form.stockDetailModelList[#rows.index].less_price'/>" class="form-control text-right"
										oninput="maxNumberLength(8)" onKeypress="return integerOnly()"/>
								</td>
                            </ww:if>
							<ww:else>
...
<!-- 							<td rowspan="2"> -->
<%--                               <input type="number" name="form.stockDetailModelList[<ww:property value='#rows.index'/>].other_price"  --%>
<%-- 									value="<ww:property value='form.stockDetailModelList[#rows.index].other_price'/>" class="form-control text-right" --%>
<!-- 									oninput="maxNumberLength(8)" onKeypress="return integerOnly()"/> -->
<!--                             </td> -->
                            <!-- sxt 20221024 del end -->
                            <td rowspan="2">
                            	<!-- sxt 20221024 add start -->
                            	<input type="hidden" name="form.stockDetailModelList[<ww:property value='#rows.index'/>].other_price" 
									value="<ww:property value='form.stockDetailModelList[#rows.index].other_price'/>" />
								<ww:if test="time_kbn.equals('')">
                            ...
                            <td class="text-center" rowspan="2">
								<ww:textarea name="'form.stockDetailModelList['+#rows.index+'].biko'" value="biko" cssClass="'form-control'" rows="3"></ww:textarea>
							</td>
<!--                             <td class="text-center" rowspan="2"> -->
...
<!--                             </td> -->
                          ...
                          <tr>
                            <td style="border-top: none;">
								<input type=text disabled name="form.stockDetailModelList[<ww:property value='#rows.index'/>].company_name" 
									value="<ww:property value='form.stockDetailModelList[#rows.index].company_name'/>" style="background-color: #ffffff;" />	 
							</td>	
                        ...
                        <tfoot>
                          <tr>
                            <td class="text-center" colspan="4"  rowspan="4"></td>
                ...
                <div class="panel panel-default">
                  <div id="myTabContent" class="tab-content">
                    <div role="tabpanel" class="tab-pane fade active in panel-body" id="detail" aria-labelledby="detail-tab">
                      ...
                      <div class="form-group form-inline">
<!--                       	<button type="button" class="btn btn-primary" onclick="submitAction(this.form,'hattyuOptionAddRow2.action');">明細行追加</button> -->
...
// 					        	}
		...
		function bingDetailEvent(){
			...
			$("input[name$='btn']").click (function(){
				...
				openWindowWithScrollbarGuide('initialSagyousyaGuide.action?form.company_code=' + code + '&form.company_name_in=' + name + '&form.fieldName_code=form.stockDetailModelList[' + index + '].task_code&form.fieldName_name=form.stockDetailModelList[' + index + '].task_content&form.company_name=form.stockDetailModelList[' + index + '].company_name&form.sagyousya_flg=2','技術者参照',850,760);	
			...
			$("input[name$='task_content']").blur(function(){
				...
				if (!$(this).val()){
	 				...
	 				//技術者コードをクリアする
	 				$("input[name='form.stockDetailModelList[" + index +"].task_code'").val("");
	 				
	 				//会社名をクリアする
	 				$("input[name='form.stockDetailModelList[" + index +"].company_name'").val("");
	 				
// 	 				//sxt 20220908 add start
// 				} else {	
// 					var task_code_old = $("input[name='form.stockDetailModelList[" + index +"].task_code_old'").val();

// 					if ($(this).val()!=task_code_old){
// 						//発注をセット
// 						setPrice(index);
// 						$("input[name='form.stockDetailModelList[" + index +"].task_code_old'").val($(this).val());
// 					}
...
// 				//sxt 20220908 add end
				...
				//空白を選択する時
				if ($("select[name='form.stockDetailModelList[" + index +"].time_kbn'").val() == ""){	
			...
			$("input[name$='price_per']").blur(function(){
				...
				//工数
				var quantity = $("input[name='form.stockDetailModelList[" + index +"].quantity'").val();
...
// 			var task_code = $("input[name='form.stockDetailModelList[" + index +"].task_code'").val();
...
// 		        	//月を選択する時
// 					if ($("select[name='form.stockDetailModelList[" + index +"].time_kbn'").val() == "01"){	
						
// 						$("input[name='form.stockDetailModelList[" + index +"].price_per'").val(month_price);
						
// 						//時を選択する時
// 					}　else if ($("select[name='form.stockDetailModelList[" + index +"].time_kbn'").val() == "02"){
...
// 						$("input[name='form.stockDetailModelList[" + index +"].price_per'").val(price);
// 					}
// 		        }
// 		    });
// 		}
// 		//sxt 20220908 add end
		...
		function setItemState(isReadOnly,index){
			if (isReadOnly){
				$("input[name='form.stockDetailModelList[" + index +"].price_per'").attr("readonly","true");
				$("input[name='form.stockDetailModelList[" + index +"].price_per'").attr("style","background-color:#fff");
				$("input[name='form.stockDetailModelList[" + index +"].price_per'").val("");
				
				$("input[name='form.stockDetailModelList[" + index +"].more_price'").attr("readonly","true");
				$("input[name='form.stockDetailModelList[" + index +"].more_price'").attr("style","background-color:#fff");
				$("input[name='form.stockDetailModelList[" + index +"].more_price'").val("");
				
				$("input[name='form.stockDetailModelList[" + index +"].less_price'").attr("readonly","true");
				$("input[name='form.stockDetailModelList[" + index +"].less_price'").attr("style","background-color:#fff");
				$("input[name='form.stockDetailModelList[" + index +"].less_price'").val("");
				
				$("input[name='form.stockDetailModelList[" + index +"].quantity'").attr("readonly","true");
				$("input[name='form.stockDetailModelList[" + index +"].quantity'").attr("style","background-color:#fff");
				$("input[name='form.stockDetailModelList[" + index +"].quantity'").val("");
				
			} else {
				$("input[name='form.stockDetailModelList[" + index +"].price_per'").removeAttr("readonly");
				$("input[name='form.stockDetailModelList[" + index +"].price_per'").removeAttr("style");
				
				$("input[name='form.stockDetailModelList[" + index +"].more_price'").removeAttr("readonly");
				$("input[name='form.stockDetailModelList[" + index +"].more_price'").removeAttr("style");
				
				$("input[name='form.stockDetailModelList[" + index +"].less_price'").removeAttr("readonly");
				$("input[name='form.stockDetailModelList[" + index +"].less_price'").removeAttr("style");
				
				$("input[name='form.stockDetailModelList[" + index +"].quantity'").removeAttr("readonly");
				$("input[name='form.stockDetailModelList[" + index +"].quantity'").removeAttr("style");
			}
		}
		//sxt 20221025 add end
...
// 			sanshoButton.attr("name","button[" + n + "]");
			...
			btn.attr("name","form.stockDetailModelList[" + n + "].btn");
// 			btn.attr("value",""); 
// 			btn.val("");
			...
			time_kbn.attr("name","form.stockDetailModelList[" + n + "].time_kbn");
			time_kbn.attr("value",""); 
			time_kbn.val("01");
			...
			more_price.attr("name","form.stockDetailModelList[" + n + "].more_price");
			...
			less_price.attr("name","form.stockDetailModelList[" + n + "].less_price");
			...
			other_price.attr("name","form.stockDetailModelList[" + n + "].other_price");
			...
			task_code.attr("name","form.stockDetailModelList[" + n + "].biko");
			task_code.attr("value",""); 
			task_code.val("");
			...
			org_price.attr("name","form.stockDetailModelList[" + n + "].company_name");
		...
		function reSetRowIndex(curIndex){
			$("input[name$='row_number']").each(function() {
				//获取当前元素的name
				...
				if (index>=curIndex){
					...
					$("input[name='form.stockDetailModelList[" + index +"].task_content'").attr("name","form.stockDetailModelList[" + (index-1) + "].task_content");
...
// 					$("input[name='button[" + index +"]'").attr("name","button[" + (index-1) + "]");
					$("input[name='form.stockDetailModelList[" + index +"].btn'").attr("name","form.stockDetailModelList[" + (index-1) + "].btn");
					
					$("select[name='form.stockDetailModelList[" + index +"].time_kbn'").attr("name","form.stockDetailModelList[" + (index-1) + "].time_kbn");
					$("input[name='form.stockDetailModelList[" + index +"].price_per'").attr("name","form.stockDetailModelList[" + (index-1) + "].price_per");
					$("input[name='form.stockDetailModelList[" + index +"].more_price'").attr("name","form.stockDetailModelList[" + (index-1) + "].more_price");
					$("input[name='form.stockDetailModelList[" + index +"].less_price'").attr("name","form.stockDetailModelList[" + (index-1) + "].less_price");
					$("input[name='form.stockDetailModelList[" + index +"].other_price'").attr("name","form.stockDetailModelList[" + (index-1) + "].other_price");
					...
					$("textarea[name='form.stockDetailModelList[" + index +"].biko'").attr("name","form.stockDetailModelList[" + (index-1) + "].biko");
					
					$("input[name='form.stockDetailModelList[" + index +"].isCheck'").attr("name","form.stockDetailModelList[" + (index-1) + "].isCheck");
					$("input[name='form.stockDetailModelList[" + index +"].company_name'").attr("name","form.stockDetailModelList[" + (index-1) + "].company_name");
					$("select[name='form.stockDetailModelList[" + index +"].tax_div_input'").attr("name","form.stockDetailModelList[" + (index-1) + "].tax_div_input");	//sxt 20230508 add 
				}
		...
		function calculatePriceAttr(index){
			...
			if ($("select[name='form.stockDetailModelList[" + index +"].time_kbn'").val() == "01"){	
				//超過単価、控除単価は支払条件で計算		
				var price_per = $("input[name='form.stockDetailModelList[" + index +"].price_per'").val();
				...
				if (price_per && work_time_to){
					...
					$("input[name='form.stockDetailModelList[" + index +"].more_price'").val(morePrice);
				}
				...
				if (price_per && work_time_from){
					...
					$("input[name='form.stockDetailModelList[" + index +"].less_price'").val(lessPrice);
				}
			//時を選択する時
			}　else if ($("select[name='form.stockDetailModelList[" + index +"].time_kbn'").val() == "02"){
		...
		function calculateKingaku(index){
			var kingaku;
			var price_per = $("input[name='form.stockDetailModelList[" + index +"].price_per'").val();
			var other_price = $("input[name='form.stockDetailModelList[" + index +"].other_price'").val();	
			var quantity = $("input[name='form.stockDetailModelList[" + index +"].quantity'").val();
...
// 				kingaku = (parseFloat(price_per) * parseFloat(standard_time) + parseFloat(other_price)) * parseFloat(quantity);	
			...
			if (kingaku != 0) {
				$("input[name='form.stockDetailModelList[" + index +"].amount'").val(kingaku);	
			} else {
		...
		function setDefaultQuantity(index){
			//時を選択する時
			if ($("select[name='form.stockDetailModelList[" + index +"].time_kbn'").val() == "02"){
				$("input[name='form.stockDetailModelList[" + index +"].quantity'").val("160"); 
			}
		}
		//sxt 20220922 add end
		...
		function ajaxSeikyuyotei(){
			...
			//var order_div = $('select[name="form.order_div_input_sinki"]').val();		//sxt 20221003 del
			var order_div = $('input[name="form.order_div_input_sinki"]').val();		//sxt 20221003 add
