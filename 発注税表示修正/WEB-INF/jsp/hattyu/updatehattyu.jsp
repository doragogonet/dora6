...
<script language="JavaScript">
	...
	function deleteUpdateHattyu(pForm){
		if (confirm("削除してよろしいでしょうか？")==true){
			document.HattyuForm.action="deleteUpdateHattyu.action";
			document.HattyuForm.submit();
		}
	}
	function deleteUpdateHattyuTOP(pForm){
		if (confirm("削除してよろしいでしょうか？")==true){
			document.HattyuForm.action="deleteUpdateHattyuTOP.action";
			document.HattyuForm.submit();
		}
	}
	...
	function deleteUpdateHattyuFromProject(pForm){
		if (confirm("削除してよろしいでしょうか？")==true){
			document.HattyuForm.action="deleteUpdateHattyuFromProject.action";
			document.HattyuForm.submit();
		}
	}
	function deleteUpdateHattyuTOPFromProject(pForm){
		if (confirm("削除してよろしいでしょうか？")==true){
			document.HattyuForm.action="deleteUpdateHattyuTOPFromProject.action";
			document.HattyuForm.submit();
		}
	}
...
<!--                   		<button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'saveUpdateHattyuTop.action');">保存</button> -->
...
<%-- 						</ww:if> --%>
		                ...
		                <!-- sxt 20220721 add start -->
						<ww:if test="form.modoruFlg.equals(\"1\")">
						    <button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'saveUpdateHattyuTopFromProject.action');">保存</button>
	                  		<ww:if test="form.stock_count==1">
...
<!-- 						<button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'refreshHattyu.action');">戻る</button> -->
						...
						<!-- sxt 20220712 add end -->
						<!-- sxt 20220721 del start -->
<!--                   		<button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'saveUpdateHattyu.action');">保存</button> -->
<%--                   		<ww:if test="form.stock_count==1"> --%>
...
<%-- 						</ww:if> --%>
						...
						<!-- sxt 20220721 add start -->
						<ww:if test="form.modoruFlg.equals(\"1\")">
						    <button type="button" class="btn btn-primary dora-sm-button" onclick="submitAction(this.form,'saveUpdateHattyuFromProject.action');">保存</button>
	                  		<ww:if test="form.stock_count==1">
						...
						<ww:if test="form.approval_div_code_update.equals('03')">
							<button type="button" class="btn btn-primary dora-sm-button"
								onclick="openWindowResizable('hattyuPrint.action?form.receive_no=<ww:property value='form.receive_no_update'/>&form.order_no_print=<ww:property value='form.order_no_update'/>','御発注書印刷',800,600);">印刷</button>
						</ww:if>
						<!-- sxt 20220721 add end -->
					</ww:else>
                </div>
...
<%--                 <div class="dora-state-zone">請求区分　<ww:property value='form.order_div_name'/></div> --%>			<!-- sxt 20221008 del -->
                ...
                <div class="dora-state-zone form-inline">
	                ...
	                <ww:select name="'form.order_div_input_update'"
						       cssClass="'form-control'" 
						       list="form.order_div_list_update" 
						       listKey="code_id" 
						       listValue="code_value" 
						       value="form.order_div_input_update" 
...
<%--                         <label class="dora-label-normal btn-link" onclick="hrefHaveParamAction('jyutyuInfo.action?form.pageFlg=7&form.receive_no=<ww:property  value="form.receive_no_update" />')"> --%>
                      ...
                      <table class="table table-bordered">
						...
						<ww:iterator value="form.stockDetailModelList_update" status="rows" id="model">
                          <tr>
                            ...
                            <td class="text-center" rowspan="2">
                              <ww:hidden name="'form.stockDetailModelList_update['+#rows.index+'].task_code'" />
				  			  <ww:textarea name="'form.stockDetailModelList_update['+#rows.index+'].task_content'" value="task_content" cssClass="'control'" rows="3"></ww:textarea>
                            </td>
                            <td class="text-center" rowspan="2" style="width:6rem;">
                              <input type="button" value="参照" class="btn btn-primary dora-tabel-button" onclick="openWindowWithScrollbarGuide('initSagyounaiyouGuide.action?form.fieldName_code=form.stockDetailModelList_update[<%=temp%>].task_code&form.fieldName_name=form.stockDetailModelList_update[<%=temp%>].task_content','作業内容参照',670,600)">
                            </td>
                            ...
                            <td class="text-center">
	                            <ww:select size="1" name="'form.stockDetailModelList_update['+#rows.index+'].unit_name_input'" 
									   cssClass="'form-control'" 
									   list="unit_name_list" 
									   listKey="code_id" 
									   listValue="code_value" 
			...
			$("#deleteRow").click(function(){
				...
				if (deleteUpdateMeisaiHattyu()) {
					$.ajax({
				        type: "POST",  
				        dataType:"html",
				        url: "deleteUpdateMeisaiHattyu.action",
				        data: $('form').serialize(),
				        error: function (error) {  
...
// 			}
		...
		function appendRow(n,rowNumber){
			...
			unit_name_input.attr("name","form.stockDetailModelList_update[" + n + "].unit_name_input");
			...
			org_price.attr("name","form.stockDetailModelList_update[" + n + "].org_price");
		...
		function reSetRowIndex(curIndex){
			...
			$("input[name^='form.stockDetailModelList_update'][name$='row_number']").each(function() {
				...
				if (index > curIndex){
					...
					$("textarea[name='form.stockDetailModelList_update[" + index +"].task_content'").attr("name","form.stockDetailModelList_update[" + (index-1) + "].task_content");
					...
					$("select[name='form.stockDetailModelList_update[" + index +"].unit_name_input'").attr("name","form.stockDetailModelList_update[" + (index-1) + "].unit_name_input");
					...
					$("select[name='form.stockDetailModelList_update[" + index +"].tax_div_input'").attr("name","form.stockDetailModelList_update[" + (index-1) + "].tax_div_input");
					$("input[name='form.stockDetailModelList_update[" + index +"].org_price'").attr("name","form.stockDetailModelList_update[" + (index-1) + "].org_price");
				}
...
// 			}	
		...
		function ajaxSeikyuyotei(){
			...
			var order_div = $('select[name="form.order_div_input_update"]').val();
